...
MLIAPDescriptorFitSNAP::MLIAPDescriptorFitSNAP(LAMMPS *_lmp, char *paramfilename) : Pointers(_lmp),  MLIAPDescriptor(_lmp)
{
  radelem = nullptr;
  wjelem = nullptr;
  snaptr = nullptr;
...
MLIAPDescriptorFitSNAP::~MLIAPDescriptorFitSNAP()
{
...
void MLIAPDescriptorFitSNAP::compute_descriptors(class MLIAPData *data)
{
  int ij = 0;
...
void MLIAPDescriptorFitSNAP::compute_forces(class MLIAPData *data)
{
...
void MLIAPDescriptorFitSNAP::compute_force_gradients(class MLIAPData *data)
{
  int ij = 0;
  for (int ii = 0; ii < data->nlistatoms; ii++) {
    const int i = data->iatoms[ii];
...
void MLIAPDescriptorFitSNAP::compute_descriptor_gradients(class MLIAPData *data)
{
  int ij = 0;
  for (int ii = 0; ii < data->nlistatoms; ii++) {
    const int ielem = data->ielems[ii];
...
void MLIAPDescriptorFitSNAP::init()
{
  snaptr->init();
}
...
void MLIAPDescriptorFitSNAP::read_paramfile(char *paramfilename)
{
...
double MLIAPDescriptorFitSNAP::memory_usage()
{
